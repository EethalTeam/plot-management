...
export default function StatusTable(props) {
  const headers = ["Color Code","Status Name",'Active']
  const TableVisibleItem = ["colorCode","statusName",'isActive']
...
const columns = [
  {
    ...
    cell: row => (
        <div>
      <input type='color' value={row.colorCode} style={{width:'30px',padding:'0px'}}/>
      </div>
    ),
    sortable: true,
    width: '30%',
  ...
  const clear = () => {
    ...
    dispatch({ type: 'text', name: '_id', value: "" });
        dispatch({ type: 'text', name: "colorCode", value: "" });
  ...
  const Validate = () => {
      if (!state.colorCode) {
    props.alert({ type: 'error', message: 'Please enter Color Code', show: true });
    return;
  }
     if (!state.statusName) {
    props.alert({ type: 'error', message: 'Please enter Status Name', show: true });
    return;
  }
         showAlert()
  }
...
const DeleteAlert = (row) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isEdit) {
        ...
        props.alert({ type: 'success', message: 'Status Updated successfully!', show: true });
      } else {
        await createStatus(saveData);
        props.alert({ type: 'success', message: 'Status created successfully!', show: true });
      }
      clear();
      getStatus();
    } catch (error) {
      throw new Error(isEdit ? 'Failed to update Status.' : 'Failed to create Status.');
    }
  };
  ...
  const editTable = (data) => {
    ...
    dispatch({ type: 'text', name: '_id', value: data._id ? data._id : '' });
        dispatch({ type: 'text', name: "colorCode", value: data.colorCode ? data.colorCode : '' });
       dispatch({ type: 'text', name: "statusName", value: data.statusName ? data.statusName : '' });
     }
  ...
  const handleView = (row) => {
    setViewData({"Color Code":row.colorCode,
 ...
 <div className="savebtndiv">
              ...
              <div className="entryfirstcolmn">
                <div >
                  <GridContainer spacing={2}>
                     ...
                     length={5}
                     onChange={(e, value) => { storeDispatch(value, "colorCode", "text") }}
                     clear={(e) => { if(e){ 
                      dispatch({type:'text',name: "colorCode",value:""});}}
                    }
                   /> 
                     ...
                     length={50}
                     onChange={(e, value) => { storeDispatch(value, "statusName", "text") }}
                     clear={(e) => { if(e){ 
                      dispatch({type:'text',name: "statusName",value:""});}}
                    }
                   /> 
          ...
          <div style={{display:'flex',width:'300px',justifyContent:'space-between'}}>
            <div className="WeeklyTaskSeacrhInput">
              ...
              />
              {searchTerm ? <ClearIcon title="clear" className="FilterClearIcon" onClick={() => {setSearchTerm('');setFilteredData(Status)}} /> : <FaSearch className="FaSearchdiv" />}
            </div>
          <ExportTableToExcel tableData={filteredData} columnConfig={columnsConfig} fileName={'Status List'} />
          <div style={{display:'flex',justifyContent:'center',alignItems:'center',width:'30px'}}>
           <StyledTooltip title={"Refresh Table"} placement="top">
          <TbRefresh onClick={()=>{if(!searchTerm){getStatus()}}}/>
          </StyledTooltip>
          </div>
            </div>
